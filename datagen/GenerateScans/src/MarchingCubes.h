...
public:
	...
	static MeshDataf doMC(const VoxelGrid& grid, float thresh, bool bExportColors, float iso = 0.0f, const mat4f* transform = NULL, bool bVerbose = true) {
...
#pragma omp parallel for
		for (int z = 0; z < grid.getDimZ(); z++) {
			for (int y = 0; y < grid.getDimY(); y++) {
				for (int x = 0; x < grid.getDimX(); x++) {
					...
					extractIsoSurfaceAtPosition(worldPos, grid, thresh, iso, results[thread]);
				}
			}
		}
		...
		if (bExportColors) meshData.m_Colors.resize(numTris * 3);
		size_t currTri = 0;
		for (size_t i = 0; i < results.size(); i++) {
			for (size_t j = 0; j < results[i].size(); j++) {
				...
				if (bExportColors) {
					meshData.m_Colors[3 * currTri + 0] = vec4f(results[i][j].v0.c, 1.0f);
					meshData.m_Colors[3 * currTri + 1] = vec4f(results[i][j].v1.c, 1.0f);
					meshData.m_Colors[3 * currTri + 2] = vec4f(results[i][j].v2.c, 1.0f);
				}
				currTri++;
			}
		}
		...
		if (bVerbose) std::cout << "merging close vertices... ";
		meshData.mergeCloseVertices(0.00001f, true);
		if (bVerbose) std::cout << "done!" << std::endl;
		if (bVerbose) std::cout << "removing duplicate faces... ";
		meshData.removeDuplicateFaces();
		if (bVerbose) std::cout << "done!" << std::endl;

		if (bVerbose) std::cout << "size after:\t" << meshData.m_Vertices.size() << std::endl;
...
private:
	...
	static void extractIsoSurfaceAtPosition(const vec3f& worldPos, const VoxelGrid& grid, float thresh, float isolevel, std::vector<Triangle>& result) {
