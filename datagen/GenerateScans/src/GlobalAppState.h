...
#define X_GLOBAL_APP_STATE_FIELDS \
	...
	X(std::string, s_outputCompletePath) \
	X(std::string, s_outputIncompletePath) \
	X(float, s_chanceDropFrames) \
	...
	X(unsigned int, s_heightPad) \
	X(unsigned int, s_maxNumScenes) \
	X(unsigned int, s_maxNumSens) \
	X(std::string, s_incompleteFramePath) \
	X(std::string, s_alnPath) \
	X(bool, s_bSaveSparse) \
	X(bool, s_bGenerateSdfs) \
	X(bool, s_bGenerateKnown) \
	X(bool, s_bUseRenderedDepth) \
	X(bool, s_bDebugVis)
